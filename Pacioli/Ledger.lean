...
total balance is `0`, every reachable ledger has total balance `0` — the
accounting equation, guaranteed by construction.
-/
...
In a double-entry system this is invariantly `0` (the accounting equation). -/
...
reachable by posting keeps total balance `0`: the accounting equation holds by
